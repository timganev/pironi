...
description: Where Outlook keeps mail and calendar on Windows, how to reach it, and what lies to you.
triggers: outlook, calendar, meetings, mailbox, inbox, mail, appointments, com, mapi, ost, pst, olk, meeting, invite, sent items, sender, senders, аутлук, календар, срещи, среща, поща, имейл, имейли, писма, писал, писали, подател, податели, кутия, пощенска
...
The Teams client's own store — where a personal-account meeting is the only copy — is
`windows-teams`. Call `read_skill` with that name rather than searching for the files.
...
None of this reaches the Teams client's own store, which needs no shell at all; that is
`windows-teams`.
